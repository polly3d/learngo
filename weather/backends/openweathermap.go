...
const (
	openWeatherMapURI = "http://api.openweathermap.org/data/2.5/forecast?%s&appid=%s&units=metric&lang=%s"
)

func (c *openWeatherMapConfig) Setup() {
	flag.StringVar(&c.apiKey, "owm-api-key", "13da7997d9f6b785e4181dc90fd01735", "Open Weather Map Api Key")
	flag.StringVar(&c.lang, "owm-lang", "en", "Language")
	flag.BoolVar(&c.debug, "owm-debug", false, "Debug")
}

func (c *openWeatherMapConfig) fetch(url string) (*openWeatherMapResponse, error) {
...
func (c *openWeatherMapConfig) parseDaily(dataInfo []dataBlock, numdays int) []iface.Day {
...
func (c *openWeatherMapConfig) parseCond(dataInfo dataBlock) (iface.Cond, error) {
	var ret iface.Cond
...
func (c *openWeatherMapConfig) Fetch(location string, numdays int) iface.Data {
	var ret iface.Data
	if len(c.apiKey) == 0 {
		log.Fatal("No Api key specified.")
	}
	location = "q=" + location
	resp, err := c.fetch(fmt.Sprintf(openWeatherMapURI, location, c.apiKey, c.lang))
	if err != nil {
...
func init() {
	iface.AllBackends["owm"] = &openWeatherMapConfig{}
}
